...
const Utils = {
    ...
    mdLiteral: (text) => String(text ?? "")
        .replace(/([\\`*~_\[\]<&|])/g, "\\$1")
        // wave25 共识(w25 glm): 行首结构符需同时覆盖**无尾随空白**的整行形态 —— `-{2,}` 单独成行
        // 是 setext 下划线(把上一行升为标题)/主题分隔线(整行内容消失), `(?=\\s)` 判据对其失效;
        // wave26 共识(w26 qwen): 补 `&`(实体引用)与 `|`(GFM 表格分隔) —— `&amp;`/`&#x3C;b&#x3E;`
        // 会被 Entity reference 规则解码, 源文字面量被静默改写; `| a |` 单独成行在 GFM 下成为表格行。
        // 两者均为 ASCII 标点, 前置反斜杠后恒为字面量。行尾判据补 `$`: 位于**字符串末尾**的单个
        // `-` 行(输出时其后的换行尚未拼接)同样构成 setext 下划线, `(?=\s)` 对 "…\n-" 失配
        // `_` 已在字符转义集内(`_{2,}`/`_x_` 均已字面化), `=`/`#` 由 `[>#=]` 分支首字转义
        // wave28 共识(w28 qwen): 连字符混合间距的主题分隔线 — `-{2,}` 分支要求其后直到行尾只有空白,
        // 而 `[-+*](?=\s|$)` 要求标记后紧跟空白, 两者对 `-- -`/`-- --`/`--  --`/`--\t--`
        // (≥3 个 `-` 夹杂空格/制表符, 合法 thematic break)整体漏配且不回溯到行首位置 ——
        // 字面文本被渲染成分隔线。新分支只匹配「行首的 `-`」, 要求整行由 `-`/空格/制表符构成、
        // 且除首个外剩余部分至少还有两个 `-`(合计 ≥3), 故 `- -`(2 个, 非任何结构)不被误伤
        .replace(/^([ \t]*)([>#=]|-{2,}(?=[ \t]*$)|[-+*](?=\s|$)|-(?=[- \t]*$)(?=(?:[- \t]*-){2})|(\d+)([.)])(?=\s|$))/gm, (m, indent, marker, num, delim) =>

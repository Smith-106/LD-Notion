...
// AgentTools — AI Agent 工具定义（Level 0-2 读写工具调度）。
// ISS-20260723-010 W6-1 (ARCH-002): 从 ai/index.js 提取 AI_AGENT_TOOLS(929-2622) 到独立模块。
// 块内 103 处 AIAssistant./3 处 AIService. 原是同文件自由变量引用，esbuild 闭包间不共享自由变量
// (coding-conventions-004)，拆分后改 lazy accessor (coding-conventions-005, ISS-007 _bridgeAccessor 同模式)：
// require("./index") 在工具执行时发生，此时 ai/index.js 已加载完，AIAssistant 已 Object.assign mixin 完毕。
// AI_AGENT_TOOLS 与 AIAssistant 双向循环（AIAssistant:622 AGENT_TOOLS 属性 + 1341/1363 工具分发白名单）经此解。
...
// lazy accessor：首次调用时 require，缓存后续查找（CommonJS require 有缓存，二次起为对象属性读）。
// AI_AGENT_TOOLS 工具 execute 非热路径，lazy 开销可接受。
let _AI = null;

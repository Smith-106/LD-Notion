...
5. 最后按 `docs/ui-regression-checklist.md` 检查 Linux.do / Notion / 知乎；通用网页剪藏以 `chrome-extension-full` 为准（油猴不再默认匹配任意网页）
...
面板里 Linux.do 和浏览器书签各有自己的来源分区，两个来源的自动导入配置互不影响。
...
> v3.17 起 GitHub 收藏源已移除（Stars/Repos/Forks/Gists 导入、GitHub 自动导入、OAuth Device Flow 一并下线）。

现在你可以把浏览器收藏并到同一个 Notion 数据库里，方便统一检索和整理。
...
| 跨源搜 xxx / 各来源有多少内容 / 推荐相似内容 | 在 Linux.do / 书签 里统一检索、统计、推荐 |
| 总结 / 头脑风暴 / 校对 / 批量翻译 / 提取为数据库 | 跑更重的 AI 工作流 |
| 导入浏览器书签 | 把外部来源导进 Notion |

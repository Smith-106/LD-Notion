...
| Stored locally | access token、refresh token、workspace meta、OAuth config。 | Integration token。 |
...
| Security note | Client Secret 会保存在本地浏览器存储，不适合共享生产级 secret。 | token 本身就是长期密钥，泄露后应在 Notion 后台轮换。 |
...
::: warning 本地凭据风险
LD-Notion 没有独立后端。OAuth Client Secret、OAuth token、manual token、AI API Key 都会保存在本地浏览器存储中。该模式适合个人自用，不适合把共享生产级 secret 放进前端配置。
:::

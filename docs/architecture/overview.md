...
7. 非敏感状态、日志和导出记录写回浏览器本地存储；OAuth 凭据自 v3.12.0 起同样保存在浏览器本地 GM 存储（跨页回调可读必需），其它敏感凭证写入本地加密保险箱。
...
- 纯前端部署：安装简单，但共享生产级 secret 仍不适合放进前端；OAuth 三键自 v3.12.0 起走浏览器本地 GM 存储以支持跨页回调，其它敏感凭证通过本地加密保险箱降低明文暴露面。

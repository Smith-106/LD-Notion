...
LD-Notion Hub v3.17.0 是 **Tampermonkey 用户脚本 + Chrome 扩展**,统一连接 Linux.do、浏览器书签、RSS、知乎 → Notion。(v3.17 起 GitHub 收藏源已移除)**纯前端,无后端/服务端,无外部数据库。**
...
| 目录 | 职责 |
| --- | --- |
| `adapter/` | 多源适配器抽象层:`SourceAdapter` 基类 + `AdapterRegistry` + LinuxDo/Bookmark/RSS/Zhihu/Generic 各适配器,新知识源接入标准接口 |
...
| `bridge/` | 浏览器书签桥接:`BookmarkBridge`、`BookmarkExporter`、`BookmarkAutoImporter`(v3.15 起 RSS 桥接已删除) |
...
| `import/` | 导入层:`AutoImporter`、`UpdateChecker`(v3.17 起 GitHub 三件套已删除) |
...
- **持久化存储键必须有 TTL 或容量上限**,禁止无界增长(GM 存储 FIFO rotate)。淘汰策略按账本性质分类(v3.14.4):**导出事实账本**(不可再生,误删致 UI 误判"待导出"→重复导出)用**容量上限淘汰**——`DedupStore` id 键源(linuxdo)、`BOOKMARK_EXPORTED` 超 10000 条才淘汰最旧,**禁止对导出账本用时间 TTL**(v3.14.3 根因:90 天窗口静默遗忘导出事实);**去重事实账本**(可再生)用 90 天时间 TTL——`DedupStore` URL 键源(bookmark/rss/zhihu/generic);日志/历史类数组用 `MAX_ENTRIES` 截断。同步投影层另有独立约束:单源行 payload ≤2000 字符(SyncLedger 硬限,超限按 ts 降序截断),id 键源同步只投递新鲜条目(ts ≥ now-90d,本地永久保留不影响)。
...
- 敏感凭证(token/Client Secret/AI Key/Obsidian token)存 GM 明文存储,审计日志由 `REDACT_IN_LOGS` 超集脱敏(v3.14.3 起保险箱退役:解锁态为页面内存态,每次加载即锁定,锁定态读空致"更新后失效")。
...
- 权限域收窄(v3.7.0, v3.17 起 GitHub+gist 站点随源移除):`@match` 显式站点(linux.do/Notion/Zhihu);`@connect` 域名白名单;`@exclude` 搜索引擎/邮箱/localhost（broad include catch-all 已移除）。

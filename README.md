...
- **修复 GitHub Device Flow 授权被 @connect 拦截**：`github.com/login/device/code` + `/login/oauth/access_token` 走 `github.com` 主域，旧白名单仅 `api.github.com`，`GM_xmlhttpRequest` 报 `not part of @connect list`。补 `build.js` 的 `// @connect github.com` 与扩展双 profile 的 `https://github.com/*`（background `ALLOWED_HOSTS` 早已含 `github.com`，不动）。验收脚本新增 R0 断言防回退。
- **验证**：`verify:github-no-callback` 9 项通过；`github-oauth-device-flow` 7 项通过；vitest 全量 111 文件/1858 用例全绿；`verify:build`/`verify:delivery` 全链通过。
...
- **可观测性泛化版本**：AI token 用量埋点（`_chatRequest`/`requestAgentChat` 新增 `onUsage` 回调提取 `result.usage`，`AgentTrace.usage` 归一化三 provider 字段差异逐次累计）；业务批量操作结构化 `BatchTrace`（泛化 AgentTrace 模式到非 AI 路径，接入书签导出与自动同步，GM FIFO rotate+脱敏，与逐项 OperationLog 互补）；测试负集第二批 61 用例，基线 1773→1834

### v3.14.33

- **M3 大文件拆分 milestone 完成（波次4-10，4/4 全部 <1500 LOC）**：`ai/index.js` 2678→1376、`ui/main-ui.js` 2980→1497、`ui/events.js` 2387→1481、`auth/index.js` 1645→1155。新增 `ui/ai-chat-ui.js`（消 ISS-016 层级倒置）、`ai/ai-classifier.js`、`ai/quick-intent.js`、`ui/github-obsidian-export.js`、`ui/events/{ai-bindings,export-bindings}.js`；统一 Object.assign/ctx mixin + lazy accessor 回边，0 加载期 require 环

### v3.14.32

- **性能与架构版本**：浏览器书签导出并发化（CONCURRENCY=3 分批并发，基线实测 2.54× 提速）；Clipper（知乎/通用页）远端对账（远端链接索引为 ground truth，换库/重建库不再误阻断）；M3 大文件拆分波次1-3（main-ui→panel-template、auth→credential-vault 达标 <1500、ai/index→ai-service）；勘误固化 0 加载期环；新增 26 用例（导出并发/远端对账/AI 子模块负集），测试基线 1747→1773
...
- **质量与维护版本**：测试基线扩至 1747 用例 / 103 文件（两轮 quality-auto-test 新增 Agent 工具与 pageCrud 编排层行为缝隙回归锁 7 场景 30 用例 + 全电池九维验收）；全库四维深审修复两处低级缺陷：书签整理 normalized 等价副本失效漏判（键域统一）、GitHub Device Flow 轮询空转（死线绑定 expires_in）

### v3.14.29

- **质量与维护版本**：测试基线扩至 1701 用例 / 98 文件（三轮 quality-auto-test 新增 35 场景：NotionAPI 传输层边界、三导入器 run() 全流程、适配器水位契约、安全原语矩阵等）；依赖安全审计清零（devDeps 工具链 5→0）；修复 `CONFIG.SCRIPT_VERSION` 版本兜底漂移（UpdateChecker 在 GM_info 缺失时的回退值已落后 12 个版本）
...
- **自动去重对账 Notion 实际状态**：GitHub/LinuxDo/Bookmark 导出前查询目标库实际「链接」内容建索引判重——远端没有就导出（账本残留不再阻断，换库/重建库假跳过修复），查询失败才降级账本；Bookmark/RSS 手建库缺属性时自举容错（首轮导出自动补建 schema）
- **GitHub 邮箱用户名守卫**：用户名栏填邮箱（高频误配）时秒级红显精确指引或建议改用 Token 认证接口，不再浪费网络往返

### v3.14.26

- **Notion 资源共享引导增强**：不可见目标警告内嵌 notion.so 直达定位链接（归一 dash-less ID）；OAuth 成功态即提示「授权 ≠ 资源共享」；工作区切换目标时警告即时重算（选中新库即消失，切页面模式自动失效），消除旧消息残留误导

### v3.14.25

- **GitHub 静默失效根因修复（odyssey-debug cycle）**：GitHub 用户名无效+未填 Token → `/users/{name}/starred` 404 被 runner 吞错致按钮绿显「完成 0 条」；修复为 401 专门分支 + 未认证用户域 404 可行动指引（修正用户名/填 Token）+ runner errors 契约——错误红显而非静默
- **四导入器 errors 契约统一**：`{importedCount, failedCount, errors}` 全出口对齐（GitHub/LinuxDo/Bookmark/RSS），配置守卫与 catch 不再吞错，`bindImportNow` 红显首条；租约竞争等跳过语义保持原样
- **LinuxDo 用户名 API 第 5 探测**：同步 DOM 探测全空时走 cookie 认证 `/session/current.json`（DOM 无关），修复论坛前端升级/CDN 缓存页导致的轮询连续失败（9/5 起实证）
- **OAuth 授权单窗口 + 刷新目标可见性交叉校验**：消除双授权弹窗；手动刷新时就地校验已配置目标可见性（`canonicalNotionId` 归一比较），不可见目标不再延迟到导出 404 才暴露
...
- Agent Loop 全路径必炸缺陷修复（v3.10.0 起）：deps getter 函数被误当对象调用——任意非帮助类 AI 消息 100% 报「处理失败: ChatState2.updateLastMessage is not a function」；7 处补 getter 调用
- 测试 +2 至 860 全绿（裸 getter 源码守卫 + deps 返回真对象断言）

### v3.14.19

- 确认对话框可见性修复（诞生缺陷）：`.ldb-confirm-overlay` 自 v2.5.0 起无 CSS 定义，对话框在 linux.do 等无限流长页面下落到视口外——清空对话/关闭面板等确认类操作表现为「按键失效」且无报错；补全屏遮罩 + 居中卡片 + 亮暗主题适配
- 撤销 toast 同型缺陷修复：删除后的撤销提示同样掉到页面底部不可见，用户删错内容无法撤销；补右下角定位 + 淡入淡出 + 5s 倒计时动画
- AI 请求/上传下载错误文案可诊断化：`[object Object]` → `Utils.formatRequestError` 提取真实网络错误（5 处）
- 测试 +3 至 858 全绿；browse-now 真实浏览器实测确认对话框居中可见

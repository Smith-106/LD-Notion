...
**背景**：用户换库/重建库后自动去重假跳过——去重与状态发现以本地账本为唯一事实源，而账本全局键(无目标库作用域)且换库不重置；GitHub/LinuxDo 不查 Notion 实际状态，与 Bookmark/RSS 的远端索引判定不对称；Bookmark/RSS 对账又硬依赖固定属性名，手建库缺属性时 400 硬失败无法自举。

- **对账式判重(远端索引 ground truth)**：新增 `NotionAPI.collectDatabaseUrls`(分页拉取目标库「链接」属性→归一 Set)；GitHub/LinuxDo/Bookmark 手动路径导出前对账——远端命中→跳过，**远端未命中→导出(账本残留不阻断)**，查询失败→降级账本(防重复轰炸)
- **手建库自举容错**：Bookmark/RSS 对账查询对属性缺失 400(validation_error) 容错为空索引——首轮导出自动补建 schema，不再永久阻断自动导入
- **GitHub 邮箱用户名精确指引**：`_wrapUserScoped404` 对邮箱形用户名给专门提示(登录邮箱≠API handle)；run() 无 Token+邮箱形用户名**快速失败**，不发网络请求
- 测试：+7 回归用例(对账三态/分页归一/自举容错/邮箱守卫)；vitest 1623/1623 全绿

## [3.14.26] - 2026-09-13

### fix (Notion 资源共享引导体验增强)

**背景**：用户反馈链路实证：Notion 集成「身份认证(OAuth/API Key)」与「资源共享(连接/Connect)」两层模型易混淆，授权成功后仍因未连接目标库而 404；且已配置目标被删除进回收站后，面板警告无法引导用户定位该库（回收站页面无连接选项，API 404 与未共享不可区分）。

- **不可见警告内嵌直达定位链接**：`buildConfiguredTargetWarning` 追加 `https://www.notion.so/{canonicalId}`（dash-less 归一），一键定位已配置库后执行连接
- **OAuth 成功态预防提示**：授权成功即提示「授权只解决身份认证，集成默认看不到任何库——目标库仍需在库页面 ••• → 连接 勾选本集成」
- **目标切换即时重算警告**：`renderWorkspaceTip` 单一来源（刷新尾 + onchange 共用）+ 暂存最近工作区数据——选中新库瞬间警告消失，切页面模式时 DB 警告自动失效，消除「警告说导出失败、验证却成功」的自相矛盾显示
- 测试：不可见警告 URL 归一断言；vitest 1615/1615 全绿
...
**背景**：用户报「加载收藏列表不能正常工作」时同时看到已存目标 404 与「✅ 获取到 0 个数据库」成功提示——两条正交路径的输出叠加：加载收藏不调 Notion，404 来自点对点 `/databases/{id}` 调用；而 0 数据库成功提示未告知用户根因（目标数据库未共享给 OAuth 集成）。

- **新增 `MSG.WORKSPACE_NO_DATABASES_HINT`**（`src/config/index.js`）：单一来源的集成共享行动指引文案
- **4 处渲染点接入**：`src/ui/events.js`（工作区目标刷新 + AI 面板）、`src/ui/generic-ui.js`、`src/ui/notion-site-ui.js`——0 数据库时就地附加指引，与 404 notFoundHint 同口径
- **4 个新测试用例**（`tests/notion-oauth.test.js`）：锁定 0 数据库场景的指引输出
...
### fix (双出口内容保真：14 轮三模型共识审计与修复)

**背景**：同一篇帖子经「Notion 出口」与「Markdown(Obsidian) 出口」导出时，可见内容必须一致。源文里的字面字符（`*` `_` `~` `` ` `` `[` `]` `<` `&` `|`、行首 `#`/`>`/`-`/`数字.`）此前会被 CommonMark/GFM 渲染器当作**结构**解析（强调、标题、引用、列表、分隔线、表格、实体引用），用户的字面内容被静默改写或丢失。本次以「每个视角三模型共同完成」的共识审计方式连续查找并修复至零发现。

- **转义链闭环**（`Utils.mdText` / `mdLiteral` / `mdLabel` / `mdUrl` / `mdLink`）：
  - 字符集补全：反引号（行内 code）、`&`（实体引用 `&amp;` / `&#x3C;b&#x3E;` 被解码）、`|`（GFM 表格分隔）
  - 行首结构符：`>` `#` `=`、`-{2,}` 整行、`-`/`+`/`*` 后跟空白或串尾、`数字.`/`数字)`；行尾判据补 `$`（位于字符串末尾的单 `-` 行仍构成 setext 下划线）；**连字符混合间距分隔线**（`-- --`、`-- -`、`--\t--` 等合计 ≥3 个 `-` 夹杂空白的合法 thematic break 此前整体漏配）
  - `mdUrl` 百分号编码 + 反斜杠转义（避免带括号的 Wikipedia 条目 404）
- **媒体容器分派**（`DOMToNotion`）：`lightbox-wrapper` / `image-wrapper` 与 `.md-table` 容器改为**按文档序逐子节点分派**——原实现把整容器交给 `_cookLightbox`，容器内 `<a class="lightbox">` 包裹的图片被整体跳过（正文丢图，与 Markdown 出口不对称）；容器内直系 emoji 图与「地址被拒 + 有 alt」图改走文本载体判定，不再把「前 🎉 后」拆成三个块
- **段落归一化**：跨片段边界空白按 HTML 空白折叠规则收敛（`<div>a <strong>b </strong> c</div>` 不再渲染出双空格，同时保留 `<br>` 产生的空行）；折叠产生的空片段不再下发（避免 Notion 400）
- **跳过表**：`object` / `embed` / `canvas` 纳入 `DomSpec.SKIP_TAGS`（此前其内容以文档对象副本进入正文）
- **Markdown 出口**：表格单元格竖线转义改为幂等 lookbehind（与行内 code 跨度口径一致）、列表项行边界与续行缩进、caption 与表头间空行、`<ol start>` / `<li value>` 支持 0/负值

### test

- 出口面契约测试：**1512 例 / 81 文件**全绿；新增 `tests/dom-exit-surface.test.js` 锁定「三个出口面共用同一判据（`DomSpec`）」与逐轮修复形态
- 变异测试：**286 条**锁定（KILLED 286 / SURVIVED 0），覆盖转义链、容器分派、归一化、截断与跳过路径
- 交付链：`node build.js` + `verify:build`（root ≡ dist）+ `verify:equivalence` + `verify:delivery` 全绿

### docs

- 新增《双出口内容保真》文档页：出口面清单、转义链规范锚点表、媒体容器分派口径与**已知边界**（记录不修项及理由）
...
- **错误分类 + 可行动文案**：新增 `src/errors/ErrorModel.js`（纯函数 `classifyError`/`annotateError`/`summarize`）——所有 Notion API 错误统一分类（认证/权限/限流/超时/服务端/资源/未知），导出报告失败区顶部附「💡 建议」下一步动作；测试断言错误分类键不锁文案（+22 用例）
- **429 重试可视化**：限流重试耗尽后错误携带 `retryCount`，文案提示「已自动重试 N 次仍被限流，请调大请求间隔」
- **401/403 →「重新授权」一键直达**：认证中止横幅新增「🔄 重新授权」按钮（OAuth 已配置时显示），点击直接打开 Notion 授权页，无需翻设置
- **Database ID 前置识别（warn-only）**：验证配置时若粘贴 notion.so 链接自动识别并提示 32 位数据库 ID，避免拿链接原文请求等 15s 超时才暴露
- **guard.denied 聚合可见**：浏览器书签 / RSS 自动同步中权限不足的跳过项独立计数（不再混入 failed），批次结束状态与通知显示「N 项因权限不足跳过（可在设置中提升权限级别）」，审计仍逐条保留
- 版本 bump 3.14.17

## [3.14.16] - 2026-09-08

### feat (认证方式显式选择 + 导出状态依据)

- **认证方式单选**：Notion 配置新增「使用 API Key（Internal）」/「使用公开 OAuth」；选择立即 `setAuthMode`，状态行显示「当前启用：…」；非活动分区弱化但仍可预填。API Key 与 OAuth 都可填写，**只有所选模式**会被导出 / `getAccessToken` 使用
- **修复脚枪**：`setManualApiKey` 保存非空 Key 不再静默翻 `manual`（单选为唯一真相源）；一键授权成功仍自动切到 OAuth
- **导出状态依据**：设置「本地账本」（默认）|「Notion 工作区」；Notion 模式只读查工作区快照链接（无快照则全部待导出并提示刷新）；提供「按 Notion 重算导出状态」（不改本地账本）
- 主面板 / Notion 站内面板 / 通用剪藏面板同步认证方式控件；版本 bump 3.14.16
